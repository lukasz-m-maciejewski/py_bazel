...
static int parse_opt(int argc, char** argv, ssh_bind sshbind) {
  ...
  if (!no_default_keys) {
    set_default_keys(sshbind, rsa_already_set, dsa_already_set,
                     ecdsa_already_set);
  }
...
static void handle_session(ssh_event event, ssh_session session) {
  ...
  struct channel_data_struct cdata = {.pid = 0,
                                      .pty_master = -1,
                                      .pty_slave = -1,
                                      .child_stdin = -1,
                                      .child_stdout = -1,
  ...
  struct session_data_struct sdata = {
      .channel = NULL, .auth_attempts = 0, .authenticated = 0};
...
static void* session_thread(void* arg) {
  ssh_session session = static_cast<ssh_session>(arg);
  ssh_event event;
